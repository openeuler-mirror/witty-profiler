"""FastAPI-based HTTP server implementation for Anansi.
...
from anansi.backend.base import Server
from anansi.backend.default_server import OnlineDisabledServer
...
if not EnvManager().fastapi_available():
    ...
    class FastAPIServer(OnlineDisabledServer):
        ...
        Wraps AnansiCore with HTTP endpoints for querying topology graphs,
        ...
        def __init__(self, core: AnansiCore):
            super().__init__(core)
        ...
        def run_online(self, addr: str, port: int):
            ...
            self._core.start()
            LOGGER.info(f"Starting Anansi server at {addr}:{port}")

            try:
                uvicorn.run(self._app, host=addr, port=port, log_level="info")
            except KeyboardInterrupt:
                LOGGER.info("Shutting down Anansi server...")
            finally:
                self._core.stop()
