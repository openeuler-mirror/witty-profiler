"""Interactive Anansi API client using `RestfulRemoteBackendClient`.

Provides an `ExampleClient` class with a `run()` loop that presents
numbered operations for interacting with the Anansi server. Host and
...
注意事项：
    - 需确保服务器先启动：`anansi` 或 `python -m anansi`。
...
from anansi.backend.backend_client.remote_restful_backend import (
...
class ExampleClient:
    """Interactive client for Anansi HTTP API.
    ...
    def run(self):
        print("Anansi API Client (interactive)")
        print("=" * 60)
        ...
        while True:
            ...
            try:
                ...
                print(f"\n❌ Error: {e}")
                print("Ensure the Anansi server is running:")
                print("  anansi")
                print("  # or: python -m anansi")
                # continue loop for more attempts


def parse_args():
    parser = argparse.ArgumentParser(description="Interactive client for Anansi API")
    parser.add_argument(
