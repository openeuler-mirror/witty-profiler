...
static std::string default_obj_path()
{
    ...
    if (len > 0) {
        ...
        if (pos != std::string::npos)
            return exe.substr(0, pos + 1) + "unix_socket_bpf.o";
    }
    return "unix_socket_bpf.o";
}
...
static int dump_uds_stats(int map_fd, int n_cpus, uint64_t window_start,
                           uint64_t window_end, bool clear_after)
{
    int entry_visited_cnt = 0;
    uds_key cur_key = {};
    std::vector<uds_stats> cpu_stats(n_cpus);

    int res = bpf_map_get_next_key(map_fd, nullptr, &cur_key);
    while (res == 0) {
        ++entry_visited_cnt;
        std::fill(cpu_stats.begin(), cpu_stats.end(), uds_stats{});
        ...
        if (bpf_map_lookup_elem(map_fd, &cur_key, cpu_stats.data()) == 0) {
            ...
            for (int i = 0; i < n_cpus; ++i) {
                ...
                if (cpu_stats[i].count > 0)
                    peer_inode = cpu_stats[i].peer_inode;
            }

            if (bytes > 0 || count > 0)
                uds_dump_emit(cur_key, window_start, window_end, bytes, count, peer_inode);
        }
...
int main(int argc, char **argv)
{
    ...
    if (print_version) {
        std::cout << "unix_socket_sniffer version: " 
                  << UNIX_SOCKET_SNIFFER_VERSION_MAJOR << "."
                  << UNIX_SOCKET_SNIFFER_VERSION_MINOR << "."
                  << UNIX_SOCKET_SNIFFER_VERSION_PATCH << std::endl;
        return 0;
    }
    ...
    bpf_map *stats_map_a = bpf_object__find_map_by_name(obj, "uds_stats_map_a");
    bpf_map *stats_map_b = bpf_object__find_map_by_name(obj, "uds_stats_map_b");

    uds_dump_print_header();
    ...
    while (!exiting) {
        ...
        int entry_cnt = dump_uds_stats(stats_fd, n_cpus, window_start, window_end, true);
