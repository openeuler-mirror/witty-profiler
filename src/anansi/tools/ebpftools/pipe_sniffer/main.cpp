...
static std::string default_obj_path()
{
    ...
    if (len > 0) {
        ...
        if (pos != std::string::npos)
            return exe.substr(0, pos + 1) + "pipe_bpf.o";
    }
    return "pipe_bpf.o";
}
...
static int dump_pipe_stats(int map_fd, int n_cpus, uint64_t window_start, 
                           uint64_t window_end, bool clear_after)
{
    int entry_visited_cnt = 0;
    pipe_key cur_key = {};
    std::vector<pipe_stats> cpu_stats(n_cpus);

    int res = bpf_map_get_next_key(map_fd, nullptr, &cur_key);
    while (res == 0) {
        ++entry_visited_cnt;
        std::fill(cpu_stats.begin(), cpu_stats.end(), pipe_stats{});
        ...
        if (bpf_map_lookup_elem(map_fd, &cur_key, cpu_stats.data()) == 0) {
            ...
            if (bytes > 0)
                pipe_dump_emit(cur_key, window_start, window_end, bytes, count);
        }
...
int main(int argc, char **argv)
{
    ...
    if (print_version) {
        std::cout << "pipe_sniffer version: " 
                  << PIPE_SNIFFER_VERSION_MAJOR << "."
                  << PIPE_SNIFFER_VERSION_MINOR << "."
                  << PIPE_SNIFFER_VERSION_PATCH << std::endl;
        return 0;
    }
    ...
    bpf_map *stats_map_a = bpf_object__find_map_by_name(obj, "pipe_stats_map_a");
    bpf_map *stats_map_b = bpf_object__find_map_by_name(obj, "pipe_stats_map_b");

    pipe_dump_print_header();
    ...
    while (!exiting) {
        ...
        int entry_cnt = dump_pipe_stats(stats_fd, n_cpus, window_start, window_end, true);
