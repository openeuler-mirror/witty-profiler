...
from anansi.entity.node_entity.key_entity import ProcessEntity
...
class IPCCollector(LocalCollector):
    ...
    def start(self):
        ...
        if self.enable_uds:
            try:
                from anansi.edge.ipc.uds_sniffer import UDSSniffer
        ...
        if self.enable_pipe:
            try:
                from anansi.edge.ipc.pipe_sniffer import PipeSniffer
        ...
        if self.enable_sysv_msg:
            try:
                from anansi.edge.ipc.sysv_msg_sniffer import SysVMsgSniffer
        ...
        if self.enable_posix_mq:
            try:
                from anansi.edge.ipc.posix_mq_sniffer import POSIXMQSniffer
        ...
        if self.enable_sysv_sem:
            try:
                from anansi.edge.ipc.sysv_sem_sniffer import SysVSemSniffer
