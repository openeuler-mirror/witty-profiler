...
5 分钟内运行 Witty Profiler 并采集第一份拓扑图。

## 前置条件

- Python 3.11+ 已安装
- Witty Profiler 已安装（参见 [Installation Guide](installation.md)）
- （可选）HTTP 服务端依赖已安装

## Step 1: 基础拓扑采集

通过 `WittyProfilerCore` 单例进行基础的编程式采集：

1. 获取 `WittyProfilerCore.get_instance()` 单例
...
```bash
python -m witty_profiler --config my_config.json
```
...
```bash
# 后台运行
nohup python -m witty_profiler --config production.json > witty_profiler.log 2>&1 &
```
...
| 端口被占用 | `python -m witty_profiler --port 9090` |
...
- **[Running Witty Profiler](../user-guide/running.md)**：完整运行指南
...
- **可视化**：使用 `witty-profiler-vis` CLI 导出图为 HTML/DrawIO/GEXF/Graphviz 格式
