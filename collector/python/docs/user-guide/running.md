...
本指南涵盖 Witty Profiler 的所有运行方式，包括命令行、编程接口和后台部署。
...
# 本地开发
python -m witty_profiler --host 127.0.0.1 --port 8000
...
# 监控指定进程
python -m witty_profiler --pid 1234

# 导出当前配置
python -m witty_profiler --dump-config local/config.snapshot.json
```

### 进程锁

Witty Profiler 通过进程文件锁（`/tmp/witty_profiler/witty_profiler_instance.lock`）防止并发运行多个实例。锁文件中记录了 PID、运行模式（online/offline）和绑定地址等元数据，报错时会提示当前占用实例的信息。
...
**流程**：
1. 启动 WittyProfilerCore（Collector 开始运行）
...
**流程**：
1. 启动 WittyProfilerCore 和 Collector
2. 运行指定时长
3. 触发最终采集
...
通过 `WittyProfilerCore` 单例进行编程式拓扑采集：

1. 获取 `WittyProfilerCore` 单例实例
...
| 方法 | 说明 |
|------|------|
| `WittyProfilerCore.get_instance()` | 获取单例 |
| `start()` | 启动采集（引用计数 +1） |
...
JSON 格式，配置 dataclass 定义在 `src/witty_profiler/config_manager/configs/` 下。

加载方式：

```bash
python -m witty_profiler --config config.json
```
...
`WITTY_PROFILER_ROOT_LOGGER_LEVEL` 可设置根日志级别。
...
创建 systemd service 文件，配置 `ExecStart` 指向 `python -m witty_profiler --config /etc/witty_profiler/config.json`，设置 `Restart=on-failure`。

### nohup

```bash
nohup python -m witty_profiler --config config.json > witty_profiler.log 2>&1 &
echo $! > witty_profiler.pid
```
...
基于 `python:3.11-slim` 镜像，安装依赖后以 `python -m witty_profiler --config configs/production.json` 作为入口，暴露 18090 端口。

## 监控与健康检查

- **HTTP 健康检查**：`GET /status` 端点返回运行状态、节点/边统计
- **日志**：通过 `--log-level` 或 `WITTY_PROFILER_ROOT_LOGGER_LEVEL` 控制
...
| 进程锁冲突 | 另一个 Witty Profiler 实例正在运行，查看锁文件中的元数据确认 |
