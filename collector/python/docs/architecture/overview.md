...
Witty Profiler 使用分层架构来发现系统拓扑，并通过 HTTP API 和订阅者回调暴露结果。
...
┌────────────────────────────┴─────────────────────────────────────┐
│                    Controller Layer                               │
│   WittyProfilerServer (Singleton) ── 后端选择与生命周期代理              │
│   WittyProfilerCore   (Singleton) ── 采集编排、订阅者通知、后台循环      │
└────────────────────────────┬─────────────────────────────────────┘
                             │
              ┌──────────────┴──────────────┐
              │                             │
...
Witty Profiler 将拓扑采集抽象为“多源观测 → 图融合 → 持续发布”的流程：
...
#### WittyProfilerCore（ThreadSafeSingleton）
...
#### WittyProfilerServer（ThreadSafeSingleton）
...
| `RemoteCollector` | 远端 Witty Profiler 实例 | 带命名空间的远程图合并 |
...
配置 dataclass 定义在 [src/witty_profiler/config_manager/configs](../../src/witty_profiler/config_manager/configs)，详见 [Configuration Guide](../configuration.md)。
