...
- If consolidating ebpftools `vmlinux.h` files, use a shared per-arch directory (for example, src/witty_profiler/tools/ebpftools/vmlinux/<arch>) referenced by all CMakeLists.txt and remember `vmlinux.h` is kernel/BTF-specific, so do not reuse across incompatible kernels.
- When refreshing documentation, verify CLI flags against `src/witty_profiler/__main__.py`, verify API envelopes against `src/witty_profiler/backend/fastapi_server.py`, and ensure doc links reflect existing files.
