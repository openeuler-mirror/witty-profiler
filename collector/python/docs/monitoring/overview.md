...
This document describes **entities (nodes)**, **relationships (edges)**, and **metrics** that Witty Profiler can detect and include in its topology graph.
...
Witty Profiler provides comprehensive IPC monitoring through multiple eBPF-based sniffers:
...
```python
from witty_profiler.tools.ebpftools import (
    PipeSniffer,
    UdsSniffer,
    SysvMsgSniffer,
    PosixMqSniffer,
...
```python
from witty_profiler.config_manager.configs.sniffer_config import IPCSnifferEnable
...
```bash
# Pipe test
sudo ./src/witty_profiler/binary/pipe/pipe_sniffer -d all -i 2
ls -la | grep "test"
...
# POSIX MQ test
sudo ./src/witty_profiler/binary/posix_mq/posix_mq_sniffer -d all -i 2
# Run posix_mq_test
