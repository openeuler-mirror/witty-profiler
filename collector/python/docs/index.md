# Witty Profiler Documentation

Welcome to documentation for **Witty Profiler**, an automated topology detection framework for AI training and inference systems.

## Overview

Witty Profiler discovers and maps inter-process communication and control dependencies, building real-time graph representations of system topology from kernel-level signals (sockets, shared memory, GPU/NPU device mapping, NUMA affinity, RDMA, CPU profiling, and IPC mechanisms).
...
- **[Running Witty Profiler](user-guide/running.md)** — CLI 用法、在线/离线模式、编程接口
...
### Visualization
- 离线图导出：`witty-profiler-vis` CLI 支持 HTML / DrawIO / GEXF / Graphviz 格式输出
...
Witty Profiler is released under Apache-2.0 License. See [LICENSE](../LICENSE) for details.
