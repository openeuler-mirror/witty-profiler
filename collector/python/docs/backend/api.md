# Witty Profiler Backend API (FastAPI)
...
| 模式 | 命令 | 说明 |
|------|------|------|
| 在线模式（默认） | `python -m witty_profiler` | 启动 HTTP 服务 |
| 自定义绑定 | `python -m witty_profiler --host 127.0.0.1 --port 9090` | 覆盖地址端口 |
| 离线批量采集 | `python -m witty_profiler --offline --duration 30` | 无 HTTP 服务，采集后输出文件 |
...
- 添加或更新端点：修改 [src/witty_profiler/backend/fastapi_server.py](../../src/witty_profiler/backend/fastapi_server.py)
- 测试：[tests/test_backend](../../tests/test_backend)
- 进程锁确保同一时间只有一个 Witty Profiler 实例运行
