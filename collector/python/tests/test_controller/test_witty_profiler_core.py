...
class TestWittyProfilerCore(unittest.TestCase):
    def setUp(self) -> None:
        WittyProfilerCore.clear_singleton()
        ...
        self._config_patcher = patch(
            "witty_profiler.controller.witty_profiler_core.GlobalConfigManager"
        )
        ...
        self._collect_set_patcher = patch(
            "witty_profiler.collector.collect_set.GlobalConfigManager"
        )
    ...
    @patch("witty_profiler.controller.witty_profiler_core.get_local_collectors")
    def test_initialization(self, mock_get_local_collectors):
        """Test WittyProfilerCore initialization."""
        mock_get_local_collectors.return_value = self.mock_collectors_dict

        core = WittyProfilerCore()
    ...
    def test_start_already_running(self, mock_get_local_collectors):
        ...
        with patch("witty_profiler.controller.witty_profiler_core.LOGGER") as mock_logger:
            core.start()
    ...
    def test_singleton_pattern(self, mock_get_local_collectors):
        """Test WittyProfilerCore follows singleton pattern."""
        mock_get_local_collectors.return_value = self.mock_collectors_dict

        core1 = WittyProfilerCore()
        core2 = WittyProfilerCore()
