...
static std::string default_obj_path()
{
    ...
    if (len > 0) {
        ...
        if (pos != std::string::npos)
            return exe.substr(0, pos + 1) + "posix_mq_bpf.o";
    }
    return "posix_mq_bpf.o";
}
...
static int dump_posix_mq_stats(int map_fd, int n_cpus, uint64_t window_start,
                                uint64_t window_end, bool clear_after)
{
    int entry_visited_cnt = 0;
    posix_mq_key cur_key = {};
    std::vector<posix_mq_stats> cpu_stats(n_cpus);

    int res = bpf_map_get_next_key(map_fd, nullptr, &cur_key);
    while (res == 0) {
        ++entry_visited_cnt;
        std::fill(cpu_stats.begin(), cpu_stats.end(), posix_mq_stats{});
        ...
        if (bpf_map_lookup_elem(map_fd, &cur_key, cpu_stats.data()) == 0) {
            ...
            for (int i = 0; i < n_cpus; ++i) {
                ...
                if (cpu_stats[i].count > 0)
                    msg_prio = cpu_stats[i].msg_prio;
            }

            if (bytes > 0 || count > 0)
                posix_mq_dump_emit(cur_key, window_start, window_end, bytes, count, msg_prio);
        }

        posix_mq_key next_key = {};
...
int main(int argc, char **argv)
{
    ...
    if (print_version) {
        std::cout << "posix_mq_sniffer version: " 
                  << POSIX_MQ_SNIFFER_VERSION_MAJOR << "."
                  << POSIX_MQ_SNIFFER_VERSION_MINOR << "."
                  << POSIX_MQ_SNIFFER_VERSION_PATCH << std::endl;
        return 0;
    }
    ...
    bpf_map *stats_map_a = bpf_object__find_map_by_name(obj, "posix_mq_stats_map_a");
    bpf_map *stats_map_b = bpf_object__find_map_by_name(obj, "posix_mq_stats_map_b");

    posix_mq_dump_print_header();
    ...
    while (!exiting) {
        ...
        int entry_cnt = dump_posix_mq_stats(stats_fd, n_cpus, window_start, window_end, true);
