...
static std::string default_obj_path()
{
    ...
    if (len > 0) {
        ...
        if (pos != std::string::npos)
            return exe.substr(0, pos + 1) + "sysv_msg_bpf.o";
    }
    return "sysv_msg_bpf.o";
}
...
static int dump_sysv_msg_stats(int map_fd, int n_cpus, uint64_t window_start,
                                uint64_t window_end, bool clear_after)
{
    int entry_visited_cnt = 0;
    sysv_msg_key cur_key = {};
    std::vector<sysv_msg_stats> cpu_stats(n_cpus);

    int res = bpf_map_get_next_key(map_fd, nullptr, &cur_key);
    while (res == 0) {
        ++entry_visited_cnt;
        std::fill(cpu_stats.begin(), cpu_stats.end(), sysv_msg_stats{});
        ...
        if (bpf_map_lookup_elem(map_fd, &cur_key, cpu_stats.data()) == 0) {
            ...
            for (int i = 0; i < n_cpus; ++i) {
                ...
                if (cpu_stats[i].count > 0)
                    msg_type = cpu_stats[i].msg_type;
            }

            if (bytes > 0 || count > 0)
                sysv_msg_dump_emit(cur_key, window_start, window_end, bytes, count, msg_type);
        }

        sysv_msg_key next_key = {};
...
int main(int argc, char **argv)
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "vp:o:d:i:")) != -1) {
        switch (opt) {
        case 'v':
            print_version = true;
            break;
    ...
    if (print_version) {
        std::cout << "sysv_msg_sniffer version: " 
                  << SYSV_MSG_SNIFFER_VERSION_MAJOR << "."
                  << SYSV_MSG_SNIFFER_VERSION_MINOR << "."
                  << SYSV_MSG_SNIFFER_VERSION_PATCH << std::endl;
        return 0;
    }
    ...
    bpf_map *stats_map_a = bpf_object__find_map_by_name(obj, "sysv_msg_stats_map_a");
    bpf_map *stats_map_b = bpf_object__find_map_by_name(obj, "sysv_msg_stats_map_b");

    sysv_msg_dump_print_header();
    ...
    while (!exiting) {
        ...
        int entry_cnt = dump_sysv_msg_stats(stats_fd, n_cpus, window_start, window_end, true);
