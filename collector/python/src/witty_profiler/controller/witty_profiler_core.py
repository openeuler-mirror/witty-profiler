...
Provides WittyProfilerCore, a thread-safe singleton that manages the lifecycle
...
Key Components:
    - WittyProfilerCore: Main singleton controller with start/stop/trigger methods
...
Lifecycle:
    1. WittyProfilerCore.get_instance() - Get singleton instance
...
from witty_profiler.collector.local_collector import get_local_collectors
from witty_profiler.collector.remote_collector import RemoteCollector
...
class WittyProfilerCore(ThreadSafeSingleton):
    """
    启动类
    """
    ...
    def __init__(self):
        self._mem_storage_subscriber = NaiveMemoryStorageGraphSubscriber(
            name="witty_profilercore-memory-subscriber"
        )
        self._sub_collection: SubscriberCollection = SubscriberCollection(
            subscribers={"witty_profiler": self._mem_storage_subscriber}
        )
        ...
        for slave_config in collect_config.remote_slaves:
            ...
            if remote_collector.is_valid():
                LOGGER.info(
                    "Remote collector %s is valid, adding to WittyProfilerCore...",
                    slave_config,
                )
    ...
    def start(self):
        with self.lock:
            if self._running:
                LOGGER.warning("WittyProfilerCore is already running.")
                self._running += 1
                return
            self._running = 1
    ...
    def stop(self):
        with self.lock:
            if not self._running:
                LOGGER.warning("WittyProfilerCore is not running.")
                return
            self._running -= 1
            if self._running > 0:
                LOGGER.info(
                    "WittyProfilerCore won't stop as there are still %d running references.",
                    self._running,
                )
                return
