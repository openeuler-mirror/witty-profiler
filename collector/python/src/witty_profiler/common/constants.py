"""Global constants for the Witty Profiler topology collection framework.
...
PKG_WITTY_PROFILER_PATH: Final[str] = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..")
)


def _get_witty_profiler_lock_dir() -> str:
    """Get the platform-appropriate directory for Witty Profiler lock files.

    Returns:
        Path to the Witty Profiler lock directory (e.g., /tmp/witty_profiler on Unix)
    """
    temp_dir = tempfile.gettempdir()
    lock_dir = os.path.join(temp_dir, "witty_profiler")
    os.makedirs(lock_dir, exist_ok=True)
    return lock_dir


class WittyProfilerServerConstants:
    """Container for Witty Profiler HTTP server constants.
    ...
    # HTTP API metadata
    API_TITLE: Final[str] = "Witty Profiler Topology API"
    API_DESCRIPTION: Final[str] = "REST API for querying system topology graphs"
    API_VERSION: Final[str] = "0.1.0"
    HELP_HEADER: Final[str] = "Witty Profiler Topology Server - Help\nUsage:\n"
...
class SocketMonitorConstants:
    ...
    def LOCK_FILE() -> str:
        ...
        return os.path.join(_get_witty_profiler_lock_dir(), "witty_profiler_socket_monitor.lock")


class WittyProfilerProcessConstants:
    @staticmethod
    def LOCK_FILE() -> str:
        """Get the lock file path for Witty Profiler process."""
        return os.path.join(_get_witty_profiler_lock_dir(), "witty_profiler_instance.lock")
...
class CacheMonitorConstants:
    ...
    def LOCK_FILE() -> str:
        ...
        return os.path.join(_get_witty_profiler_lock_dir(), "witty_profiler_cache_monitor.lock")
...
class SchedMonitorConstants:
    ...
    def LOCK_FILE() -> str:
        ...
        return os.path.join(_get_witty_profiler_lock_dir(), "witty_profiler_sched_monitor.lock")
