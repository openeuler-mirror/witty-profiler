...
Provides centralized logging setup for Witty Profiler framework with custom
...
Usage:
    ...
    file_logger = log_manager.get_logger(
        name="file_logger",
        log_file_path="/tmp/witty_profiler.log"
    )

    # Initialize from JSON
    log_manager = LogManager("/etc/witty_profiler/logging.json")
    ```
...
class LogManager:
    ...
    def _init_root_logger(self):
        ...
        root_logger_level = os.environ.get(
            "WITTY_PROFILER_ROOT_LOGGER_LEVEL", DEFAULT_LOG_LEVEL
        )
        try:
            ...
            print(
                "Invalid env variable $WITTY_PROFILER_ROOT_LOGGER_LEVEL: {}, using REPORT instead. {}".format(
                    root_logger_level,
                    e,
                )
            )
        ...
        root_logger.log(
            ...
            % (logging.getLevelName(root_logger_level), "WITTY_PROFILER_ROOT_LOGGER_LEVEL"),
        )
