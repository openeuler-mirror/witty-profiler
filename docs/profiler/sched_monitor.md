...
- 用户态加载器：`src/anansi/tools/ebpftools/sched_monitor_c/main.cpp`
- eBPF 源：`src/anansi/tools/ebpftools/sched_monitor_c/ebpf/sched_monitor.bpf.c`
- 输出逻辑：`src/anansi/tools/ebpftools/sched_monitor_c/dump/`
- 构建脚本：`src/anansi/tools/ebpftools/sched_monitor_c/CMakeLists.txt`
...
# 手动构建（sched_monitor）
cmake -B src/anansi/binary/cpu_sched -S src/anansi/tools/ebpftools/sched_monitor_c -DSCHED_OUTPUT_MSGSPEC=OFF
cmake --build src/anansi/binary/cpu_sched
```
...
```bash
sudo src/anansi/binary/cpu_sched/sched_monitor [-v] [-t <TID...>] [-p <TGID...>] [-cpu <CPU...>] [-i <SECONDS>] [-d <SECONDS>] [-m <MAX_ENTRY>] [-o <BPF_OBJ>]
```
