...
### Binary
```bash
./src/anansi/binary/sysv_msg/sysv_msg_sniffer [options]
```
...
# Monitor only sends
sudo ./src/anansi/binary/sysv_msg/sysv_msg_sniffer -d send -i 2
```
...
```python
from anansi.tools.ebpftools.sysv_msg_sniffer import SysvMsgSniffer
