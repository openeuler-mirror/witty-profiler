...
- 用户态加载器：`src/anansi/tools/ebpftools/socket_sniffer_c/main.cpp`
- eBPF 源：`src/anansi/tools/ebpftools/socket_sniffer_c/ebpf/*.bpf.c`
- eBPF 公共头：`src/anansi/tools/ebpftools/socket_sniffer_c/ebpf/socket_sniffer_common.h`
- LRU 逻辑：`src/anansi/tools/ebpftools/socket_sniffer_c/lru/`
- 输出逻辑：`src/anansi/tools/ebpftools/socket_sniffer_c/flowdump/`
...
# 手动构建（socket_sniffer）
cmake -B src/anansi/binary/socket -S src/anansi/tools/ebpftools/socket_sniffer_c -DSOCKET_LRU_DYNAMIC=OFF -DSOCKET_OUTPUT_MSGSPEC=OFF
cmake --build src/anansi/binary/socket
```

产物：

- `src/anansi/binary/socket/socket_sniffer_bpf.o`（eBPF 对象）
- `src/anansi/binary/socket/socket_sniffer`（用户态加载器）

## 运行

```bash
sudo src/anansi/binary/socket/socket_sniffer -p <PID> -d send|recv|all -m <entries> -i <sec> -o src/anansi/binary/socket/socket_sniffer_bpf.o
```
...
- 仅 IPv4；IPv6 未实现。
- CMake 会自动识别 `CMAKE_SYSTEM_PROCESSOR`，为 eBPF 编译传入正确的 `__TARGET_ARCH_*` 宏，并优先从 `src/anansi/tools/ebpftools/vmlinux/<arch>/vmlinux.h` 包含（无则回退到 `src/anansi/tools/ebpftools/vmlinux/vmlinux.h`）。
