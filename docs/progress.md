...
## 2026-01-29
- Completed skill documentation and assets for topology perception under skill/anansi-topolody-perception-skill.
...
- Pointed ebpftools CMake vmlinux.h lookup to shared src/anansi/tools/ebpftools/vmlinux/<arch> path.
...
- Verify runtime `sys.path` and module origin to ensure `anansi` is imported from a single package root when using the CLI.

## 2026-02-06
- Refreshed documentation to align with current CLI flags, config schema, API response envelope, and offline output behavior.
- Updated profiler guides to use shared ebpftools vmlinux path and the `anansi-build` workflow.
...
## 2026-02-07
- Added a CLI-level process lock to prevent concurrent Anansi runs and surface a clear error when another instance is active.
- Enhanced the CLI process lock to persist pid/mode/host/port metadata and report it in the lock error message.

## 2026-03-03
- Added a new visualization architecture under `src/anansi/visualize/` using a two-stage pipeline: `Graph -> LayoutGraph -> Renderer`.
...
- Added `html` and `drawio` renderers under `src/anansi/visualize/renderer/` that consume layout geometry and generate SVG-based HTML and draw.io `mxfile` XML outputs.
