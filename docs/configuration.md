...
本指南描述 Anansi 的运行时配置系统、JSON 配置结构和 CLI 覆盖机制。
...
```bash
anansi --dump-config local/config.snapshot.json
```

## 配置文件格式

配置文件为 JSON 格式，对应的 dataclass 定义在 [src/anansi/config_manager/configs](../src/anansi/config_manager/configs) 目录下。
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `tmp_dir` | str | `"local/anansi/run/"` | 临时文件和离线输出目录 |
...
定义在 [server_config.py](../src/anansi/config_manager/configs/server_config.py)。
...
定义在 [collector_config.py](../src/anansi/config_manager/configs/collector_config.py)。
...
定义在 [sniffer_config.py](../src/anansi/config_manager/configs/sniffer_config.py)。
...
| `data_file_path` | `"anansi.socket_sniffer.csv"` | 数据文件路径 |
...
| `cache_data_file_path` | `"anansi.cache_miss.csv"` | Cache 数据文件 |
| `sched_data_file_path` | `"anansi.sched_monitor.csv"` | 调度数据文件 |
| `entry_buffer_size` | `20000` | 缓冲区大小 |
...
完整 CLI 说明参见 [Running Anansi](user-guide/running.md)。
...
```json
{
  ...
  "sniffer_config": {
    ...
    "cpu_sniffer": {
      "cache_miss_monitor_binary_path": "src/anansi/binary/cache_miss/cache_miss_monitor",
      "cpu_sched_monitor_binary_path": "src/anansi/binary/cpu_sched/sched_monitor"
    },
    "gpu_sniffer": {
